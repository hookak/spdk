...
raid_bdev_channel_remove_base_bdev(struct spdk_io_channel_iter *i)
{
	...
	if (raid_ch->base_channel[idx] != NULL) {
		struct spdk_io_channel *tmp_ch = raid_ch->base_channel[idx];
		struct spdk_bdev *base_bdev = NULL;
		if (base_info && base_info->desc) {
			base_bdev = spdk_bdev_desc_get_bdev(base_info->desc);
		}

		/* Drain outstanding IOs before finally releasing the channel. */
		raid_bdev_wait_and_put_channel(base_bdev, tmp_ch);
